...
class Main_GUI:
    ...
    video_name = '../Videos/Testing_video_original-' + datetime.now().strftime("%m%d_%H-%M-%S") + '.mp4'
    ...
    def __init__(self):
        ...
        self.record_origin = cv2.VideoWriter(self.video_name, fourcc, 10.0, (640, 480))
    ...
    def Random_Mode(self):
        if self.Random_mode_run_time == 0:
    ...
    def Tracking_Mode(self):
        if self.Tracking_mode_run_time == 0:
            if self.Tracking_mode_init_time == 0:
                self.Tracking_mode_init_time = time.time()
            elif time.time() - self.Tracking_mode_init_time > 7:
                print('Start')
                self.Tracking_Flag = True
                self.Tracking_mode_run_time += 1
                self.Tracking_mode_random_time = random.randint(2, 5)
                print('Scroll')
                self.Serial.Transmit_value('Scroll', self.Scroll_value)
                self.Tracking_mode_init_time = time.time()
        elif self.Tracking_mode_run_time == 1:
            if time.time() - self.Tracking_mode_init_time > self.Tracking_mode_random_time:
                print('Shot', self.Tracking_mode_random_time)
                self.shoot()
                self.Tracking_mode_init_time = time.time()
                self.Tracking_mode_random_time = random.randint(2, 5)
        if self.Tracking_mode_run_time == -1:
            print('Stop Shot')
            self.Serial.Transmit_value('SRotate')
            time.sleep(0.005)
            self.Serial.Transmit_value('Stop')
            ...
            self.Tracking_Flag = False
            self.Tracking_mode_init_time = 0
            self.Tracking_Mode_Button.after_cancel(self.Tracking_Mode)
        else:
            self.Tracking_Mode_Button.after(1, self.Tracking_Mode)
