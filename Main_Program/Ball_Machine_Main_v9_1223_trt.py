...
class Main_GUI:
    ...
    trt_path = './Person_Training-yolov4_tiny_288-1214-coco.trt'
    ...
    def Random_Mode(self):
        if self.Random_mode_run_time == 0 and self.Training_Flag:
            ...
            # print('Rotate Right')
            self.Serial.Transmit_value('Right', 300)
            ...
            if self.Random_mode_init_time == 0:
                self.Random_mode_init_time = time.time()
                # print('Rotate Left')
                self.Serial.Transmit_value('Left', 300)
            elif time.time() - self.Random_mode_init_time > 1:  # << delay
                # print('Stop Rotate')
                self.Serial.Transmit_value('SRotate')
                time.sleep(0.01)
                self.shoot()
    ...
    def shoot(self):
        if self.shoot_timer == 0:
            ...
            self.Shoot_Button.after(1, self.shoot)
        else:
            if time.time() - self.shoot_timer >= 0.7:
                self.Serial.Transmit_value('ShSot')
                time.sleep(0.001)
        ...
        if point_x != 0 and point_y != 0:
            ...
            if gap < -self.scale_gap:
                if not self.turn_right_flag:
                    ...
                    self.Serial.Transmit_value('Right', int(self.Rotate_value))
            elif gap > self.scale_gap:
                if not self.turn_Left_flag:
                    self.turn_Left_flag = True
                    self.Serial.Transmit_value('Left', int(self.Rotate_value))
            else:
    ...
    def Manual_UI(self):
        ...
        self.Shoot_Button = tk.Button(self.main_window, text='Shoot', font=('Times', 16, 'bold'),
                                      ...
                                      command=self.shoot)
        # lambda msg='Shot': self.Send_Command(msg)
    ...
    def Training_UI(self):
        ...
        self.degree_hard_button.place(x=665, y=degree_y + 50)
        ...
        self.degree_medium_button.place(x=785, y=degree_y + 50)
        ...
        self.degree_easy_button.place(x=905, y=degree_y + 50)
