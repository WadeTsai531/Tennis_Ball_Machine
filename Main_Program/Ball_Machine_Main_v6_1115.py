...
class Main_GUI:
    ...
    # './Person_Training_yolov4-tiny-L3_288-1115_all.trt'
    video_name = './Video/Testing_video_original-' + datetime.now().strftime("%m%d_%H-%M-%S") + '.mp4'
    ...
    def __init__(self):
        ...
        # -------- Serial Setup --------
        self.Serial = Serial_src.Serial_System()
        ...
        # self.record_origin = cv2.VideoWriter(self.video_name, fourcc, 20.0, (640, 480))
        # self.record_result = cv2.VideoWriter(self.video_name.replace('original', 'result'), fourcc, 20.0, (640, 480))
        self.Yolov4_setup()
    ...
    def Random_Mode(self):
        if self.Random_mode_run_time == -1:
            ...
            print('Scroll')
            self.Serial.Transmit_value('Scroll', 250)
            print('Rotate Right')
    ...
    def Re_Vision(self):
        ret, frame = self.cam.read()
        # self.record_origin.write(frame)

        if self.Tracking_Test_Flag:
            frame = self.Tracking(frame)
        # self.record_result.write(frame)
        frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        frame_PIL = Image.fromarray(frame)
        frame_tk = ImageTk.PhotoImage(frame_PIL)
        ...
        if point_x != 0 and point_y != 0:
            ...
            if gap < -80:
                ...
                if not self.turn_right_flag:
                    ...
                    self.Serial.Transmit_value('Right', int(self.Rotate_Scale.get() + (abs(gap) - 80)))
                    print('Turn Right -- {}'.format(int(self.Rotate_Scale.get() + (abs(gap) - 80))))
                elif (abs(gap) - self.old_gap) / self.old_gap > 0.05:
                    self.old_gap = abs(gap)
                    self.Serial.Transmit_value('Right', int(self.Rotate_Scale.get() + (abs(gap) - 80) * 5))
                    print('Turn Right -- {}'.format(int(self.Rotate_Scale.get() + (abs(gap) - 80))))
            elif gap > 80:
                status = 'Left'
                if not self.turn_Left_flag:
                    self.turn_Left_flag = True
                    self.Serial.Transmit_value('Left', int(self.Rotate_Scale.get() + (abs(gap) - 80)))
                    print('Turn Left')
                elif (abs(gap) - self.old_gap) / self.old_gap > 0.05:
                    self.old_gap = abs(gap)
                    self.Serial.Transmit_value('Left', int(self.Rotate_Scale.get() + (abs(gap) - 80)))
                    print('Turn Left -- {}'.format(int(self.Rotate_Scale.get() + (abs(gap) - 80))))
            else:
                status = 'Stop'
    ...
    def Close_Window(self):
        ...
        self.cam.release()
        # self.record_origin.release()
        # self.record_result.release()
