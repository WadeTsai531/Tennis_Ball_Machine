...
class Vision_System:
    def __init__(self):
        ...
        model_path = 'yolov4-tiny-288.trt'
    ...
    def Start_Vision(self, window, x=5, y=5):
        ...
        self.Tracking_Button = tk.Button(window, text='Start Tracking',
                                         ...
                                         command=self.Enable)
        self.Tracking_Button.place(x=x + 660, y=y + 40)
    ...
    def Tracking(self, frame):
        boxes, scores, classes = self.trt_yolo.detect(frame, 0.5)
        # cv2.rectangle(frame,
        ...
        if point_x != 0 and point_y != 0:
            ...
            if gap < -80:
                if not self.turn_right_flag:
                    Serial.Transmit_value('Left', 220)
                    self.turn_right_flag = True
                    print('Turn Right')
            else:
                if self.turn_right_flag:
                    Serial.Transmit_value('SRotate')
                    self.turn_right_flag = False
                    print('Stop')

            if gap > 80:
                if not self.turn_Left_flag:
                    Serial.Transmit_value('Right', 220)
...
class Control_GUI:
    ...
    def Start_GUI(self, window, x=0, y=0):
        ...
        self.show_value_label = tk.Label(window, text='Shoot: 0 -- Rotate: 0',
                                         font=('Times', 15, 'bold'))
        self.show_value_label.place(x=x - 50, y=y - 50)

        self.Rotate_Calibration_Button = tk.Button(window, text='Calibration', width=9,
                                                   font=('Times', 14, 'bold'), bg='#00E088', activebackground='#00E088',
                                                   command=self.Calibration)
        self.Rotate_Calibration_Button.place(x=10, y=505)
    ...
    def Scale(self, a):
        ...
        if not self.shoot_button_flag:
            Serial.Transmit_value('Scroll', self.old_Shoot_value)
    ...
    def Up(self):
        print('Up')
        Serial.Transmit_value('Up')

    def Down(self):
        print('Down')
        Serial.Transmit_value('Down')

    def Raise_Stop(self):
        print('Raise Stop')
        Serial.Transmit_value('SRaise')


class Main:
    def __init__(self):
        # -------- Variable --------
        self.main_window_x = 1024
        self.main_window_y = 600
        ...
        self.Frame_Track.place(x=651, y=5)
        self.Canvas_Track = tk.Canvas(self.Frame_Track, width=295, height=100)
        self.Canvas_Track.pack()
        Label_Track = tk.Label(self.main_window, text='Tracking Plane',
                               font=('Times', 14, 'bold'))
        Label_Track.place(x=660, y=8)

        # -------- Control Plane --------
        control_x, control_y, control_w, control_h = 660, 160, 295, 270
